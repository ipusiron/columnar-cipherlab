...
平文を長方形（マトリクス）の行方向に書き入れ、列（column）を鍵順で並べ替えて縦に書き出すことで、暗号分を作る。
...
本ツールでは、デフォルトの平文としてマザーグース童話の「誰がコマドリを殺したか」の一節が設定されています：
> "Who killed Cock Robin? I, said the Sparrow,"

この有名な童話詩を題材に、縦列転置式暗号の仕組みを体験できます。
...
### 暗号化
「暗号化」タブで平文を入力。デフォルトでマザーグース童話の一節「Who killed Cock Robin? I, said the Sparrow,」が入力されています。
...
- キーワード：
  ...
  - 例："BALLOON"⇒2134567
- 数列：
  - 1からnまでの連続した整数のみ許可（順序は自由）
  - 例: 「3 1 4 2」「2,4,1,3,5」など
  - 重複や欠番は不可
「暗号化」押下で暗号文とグリッドを表示。
※平文と鍵が両方入力されている場合のみ暗号化ボタンが有効になります。
...
暗号化マトリクス（以下、マトリクスと略す）に、以下のメイン操作がある。

- （マトリクスに）平文文字を「書き込み」
- （マトリクスから）暗号文文字を「読み出し」（＝「書き出し」）

鍵があれば、マトリックスる列ごとにシャッフルしてから、書き出す。
